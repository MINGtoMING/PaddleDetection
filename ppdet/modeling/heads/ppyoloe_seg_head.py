...
class PPYOLOESegHead(nn.Layer):
    ...
    def forward_eval(self, feats):
        ...
        for i, feat in enumerate(feats):
            ...
            mask_coeff_list.append(mask_coeff.reshape([-1, self.num_protos, l]))
    ...
    @staticmethod
    def _dice_loss(inputs, targets, mask, num_gts):
    ...
    def _mask_loss(self, pred_coeffs, prototypes, assigned_labels,
                   assigned_bboxes, assigned_gt_index, gt_masks, stride_tensor):
        ...
        if mask_positive.sum() > 0:
            ...
            for i in range(batch_size):
                if mask_positive[i].sum() > 0:
                    ...
                    single_loss_mask *= valid_mask
                    single_loss_mask = single_loss_mask.sum() / valid_mask.sum()

                    single_loss_dice = self._dice_loss(
                        pos_pred_mask, pos_assigned_mask,
                        valid_mask, mask_positive[i].sum())

                    loss_mask += single_loss_mask
                    loss_dice += single_loss_dice
    ...
    def get_loss_from_assign(self, pred_scores, pred_distri, pred_bboxes, pred_coeffs,
        ...
        loss_mask, loss_dice = self._mask_loss(pred_coeffs, prototypes, assigned_labels,
                                               assigned_bboxes, assigned_gt_index,
                                               gt_masks, stride_tensor)
    ...
    def post_process(self, head_outs, scale_factor):
        ...
        if self.exclude_post_process:
            ...
            if self.exclude_nms:
                ...
                return pred_bboxes, pred_scores, pred_coeffs, prototypes, None
            else:
                ...
                coeff_pred = paddle.gather(
                    pred_coeffs.flatten(0, 1), nms_keep_idx, axis=0)
                coeff_pred = coeff_pred.reshape([batch_size, -1, self.num_protos])
                ...
                mask_pred = (mask_pred > 0).astype(paddle.int32)
