...
from ..bbox_utils import batch_distance2bbox, bbox_center
...
class MaskFeatHead(nn.Layer):
    def __init__(self,
        ...
        self.fusion_conv = nn.Conv2D(sum(in_channels), feat_dim, 1)
        stacked_convs = []
        for _ in range(num_stacked_convs):
            stacked_convs.append(
                ConvBNLayer(feat_dim, feat_dim, 3, padding=1, act=act))
        self.stacked_convs = nn.Sequential(*stacked_convs)
        self.output_conv = nn.Conv2D(feat_dim, num_prototypes, 1)

    def forward(self, feats, order='low2high'):
        assert len(feats) == len(self.in_channels)
        if order == 'low2high':
            max_size = feats[-1].shape[-2:]
        else:
            max_size = feats[0].shape[-2:]
        inter_feats = []
        for feat in feats:
            if feat.shape[-2:] != max_size:
                ...
                inter_feat = feat
            inter_feats.append(inter_feat)
        inter_feats = paddle.concat(inter_feats, axis=1)
        fusion_feats = self.fusion_conv(inter_feats)
        mask_feats = self.stacked_convs(fusion_feats)
        mask_feats = self.output_conv(mask_feats)
        return mask_feats
...
class PPYOLOEInstHead(nn.Layer):
    ...
    def __init__(self,
        ...
        for in_c in self.in_channels:
            ...
            self.stem_kernel.append(
                ConvBNLayer(in_c, in_c, 3, padding=1, act=act))
        # pred head
        self.pred_cls = nn.LayerList()
        self.pred_reg = nn.LayerList()
        self.pred_kernel = nn.LayerList()
        for in_c in self.in_channels:
            ...
            self.pred_kernel.append(
                nn.Conv2D(
                    in_c, self.num_gen_params, 3, padding=1))
        # projection conv
        self.proj_conv = nn.Conv2D(self.reg_max + 1, 1, 1, bias_attr=False)
        self.proj_conv.skip_quant = True
        # mask head
        self.mask_head = MaskFeatHead(
            in_channels, num_prototypes=num_prototypes, act=act)
        self._init_weights()
    ...
    def forward_train(self, feats, targets):
        ...
        cls_score_list, reg_distri_list, kernel_param_list = [], [], []
        for i, feat in enumerate(feats):
            avg_feat = F.adaptive_avg_pool2d(feat, (1, 1))
            cls_logit = self.pred_cls[i](self.stem_cls[i](feat, avg_feat) +
                                         feat)
            reg_distri = self.pred_reg[i](self.stem_reg[i](feat, avg_feat))
            kernel_param = self.pred_kernel[i](self.stem_kernel[i](feat))
            # cls and reg
            cls_score = F.sigmoid(cls_logit)
            cls_score_list.append(cls_score.flatten(2).transpose([0, 2, 1]))
            reg_distri_list.append(reg_distri.flatten(2).transpose([0, 2, 1]))
            kernel_param_list.append(kernel_param.flatten(2).transpose([0, 2, 1]))
        cls_score_list = paddle.concat(cls_score_list, axis=1)
        reg_distri_list = paddle.concat(reg_distri_list, axis=1)
        kernel_param_list = paddle.concat(kernel_param_list, axis=1)

        return self.get_loss([
            cls_score_list, reg_distri_list, kernel_param_list, mask_feat,
            anchors, anchor_points, num_anchors_list, stride_tensor
        ], targets)
    ...
    def _parse_dynamic_params(self, flatten_kernels):
        ...
        params_splits = list(paddle.split(
    ...
    def _mask_iou(self, inputs, targets):
        inputs = (F.sigmoid(inputs) > 0.5).astype('float32')
        inputs = inputs.flatten(1)
        targets = targets.flatten(1)
    ...
    def _mask_loss(self, pred_kernels, mask_feats, assigned_bboxes,
                   pos_assigned_masks, pos_keep_indexes, anchor_points):
        batch_size, _, mask_h, mask_w = mask_feats.shape
        coord = anchor_points[-mask_h * mask_w:].reshape([1, -1, 2])
        ...
        for i in range(batch_size):
            ...
            if pos_keep_index is not None:
                ...
                pos_pred_kernel = pred_kernels[i][pos_keep_index].reshape(
                    [-1, self.num_gen_params])
                pos_assigned_bbox = assigned_bboxes[i][pos_keep_index]
                pos_prior = bbox_center(pos_assigned_bbox).reshape([-1, 1, 2])
                relative_coord = (pos_prior - coord).reshape([-1, mask_h, mask_w, 2]) / 64.
                relative_coord = relative_coord.transpose([0, 3, 1, 2])
                mask_feat = mask_feats[i].unsqueeze(0).tile([num_inst, 1, 1, 1])
                mask_feat = paddle.concat([relative_coord, mask_feat], axis=1)
                weights, biases = self._parse_dynamic_params(pos_pred_kernel)
                n_layers = len(weights)
                x = mask_feat.reshape([1, -1, mask_h, mask_w])
                for j, (weight, bias) in enumerate(zip(weights, biases)):
                    x = F.conv2d(
                        x, weight, bias=bias, stride=1, padding=0, groups=num_inst)
                    if j < n_layers - 1:
                        x = F.relu(x)
                pos_pred_mask = x.reshape([num_inst, mask_h, mask_w])
                pos_pred_mask = F.interpolate(
                    pos_pred_mask.unsqueeze(0),
                    scale_factor=4,
                    mode='bilinear',
                ...
                mask_iou += self._mask_iou(pos_pred_mask, pos_assigned_mask)
                loss_dice += self._dice_loss(pos_pred_mask, pos_assigned_mask, num_inst)

        mask_iou /= batch_size
        loss_dice /= batch_size
        return mask_iou, loss_dice

    def get_loss(self, head_outs, gt_meta):
        pred_scores, pred_distri, pred_kernels, mask_feats, anchors, \
        ...
        mask_iou, loss_dice = self._mask_loss(pred_kernels, mask_feats, assigned_bboxes * stride_tensor,
                        pos_assigned_masks, pos_keep_indexes, anchor_points)

        loss = self.loss_weight['class'] * loss_cls + \
               self.loss_weight['iou'] * loss_iou + \
               self.loss_weight['dfl'] * loss_dfl + loss_dice * 2.5
        out_dict = {
            'loss': loss,
            'loss_cls': loss_cls,
            'loss_iou': loss_iou,
