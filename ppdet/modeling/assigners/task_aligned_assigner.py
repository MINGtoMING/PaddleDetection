...
__all__ = ['TaskAlignedAssigner', 'SegTaskAlignedAssigner']
...
@register
class SegTaskAlignedAssigner(nn.Layer):
    """TOOD: Task-aligned One-stage Object Detection
    """
    ...
    def __init__(self,
        ...
        super(SegTaskAlignedAssigner, self).__init__()
        self.topk = topk
        self.alpha = alpha
        self.beta = beta
        self.eps = eps
    ...
    def forward(self,
        ...
        pos_gt_masks, pos_pred_masks, pos_assigned_bboxes = \
            self._gen_mask_targets(
                gt_bboxes, gt_masks, pred_coeffs, prototypes,
                assigned_gt_index, mask_positive_sum)
        ...
        return assigned_labels.detach(), assigned_bboxes.detach(), \
            assigned_scores.detach(), pos_gt_masks, pos_pred_masks, pos_assigned_bboxes

    def _gen_mask_targets(self, gt_bboxes, gt_masks, pred_coeffs, prototypes,
                          assigned_gt_index, mask_positive_sum):
        pos_assigned_masks = []
        pos_pred_masks = []
        pos_assigned_bboxes = []
        for batch_idx in range(gt_bboxes.shape[0]):
            pos_assigned_gt_index = \
                assigned_gt_index[batch_idx][mask_positive_sum[batch_idx] > 0]
            if len(pos_assigned_gt_index) == 0:
                continue
            if len(pos_assigned_gt_index) > self.max_masks_to_train:
                pos_assigned_gt_index = \
                    pos_assigned_gt_index[:self.max_masks_to_train]
            pos_assigned_mask = paddle.gather(
                gt_masks[batch_idx], pos_assigned_gt_index, axis=0)
            pos_assigned_bbox = paddle.gather(
                gt_bboxes[batch_idx], pos_assigned_gt_index, axis=0)
            pos_pred_coeff = paddle.gather(
                pred_coeffs[batch_idx], pos_assigned_gt_index, axis=0)
            prototype = prototypes[batch_idx]
            pos_pred_mask = paddle.einsum(
                'phw,np->nhw', prototype, pos_pred_coeff)
            pos_assigned_mask = F.interpolate(
                pos_assigned_mask.unsqueeze(0),
                size=pos_pred_mask.shape[-2:],
                mode='bilinear',
                align_corners=False).squeeze(0)
            pos_assigned_masks.append(pos_assigned_mask.detach())
            pos_pred_masks.append(pos_pred_mask)
            pos_assigned_bboxes.append(pos_assigned_bbox.detach() / 4.)

        return pos_assigned_masks, pos_pred_masks, pos_assigned_bboxes
