...
class InstATSSAssigner(nn.Layer):
    ...
    def forward(self,
        ...
        Args:
            ...
            gt_masks (Tensor, uint8): Ground truth masks, shape(B, n, mask_h, mask_w)
                shape(B, L, P)
            pad_gt_mask (Tensor, float32): 1 means bbox, 0 means no bbox, shape(B, n, 1)
            bg_index (int): background index
            gt_scores (Tensor|None, float32): Score of gt_bboxes, shape (B, n, 1),
                if None, then it will initialize with one_hot label
        ...
        Returns:
            ...
            pos_assigned_masks (List): [(num_inst, mask_h, mask_w), ...]
            pos_keep_indexes (List): [(num_inst, ), ...]
        """
        assert gt_labels.ndim == gt_bboxes.ndim and \
               gt_bboxes.ndim == 3 and gt_masks.ndim == 4
        ...
        # 3. on each pyramid level, selecting topk the closest candidates
        ...
        for i in range(batch_size):
            if mask_positive_sum[i].sum() > 0:
                pos_keep_index = paddle.nonzero(mask_positive_sum[i]).flatten()
                pos_assigned_index = assigned_gt_index[i][pos_keep_index]
                pos_assigned_mask = paddle.gather(
                    gt_masks[i], pos_assigned_index, axis=0)

                pos_assigned_masks.append(pos_assigned_mask)
                pos_keep_indexes.append(pos_keep_index)
            else:
                pos_assigned_masks.append(None)
                pos_keep_indexes.append(None)
