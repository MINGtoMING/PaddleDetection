...
class ContrastiveEmbed(nn.Layer):
    ...
    def __init__(self, visual_embed_dim, text_embed_dim, proj_dim):
        super(ContrastiveEmbed, self).__init__()
        self.visual_proj = nn.Linear(visual_embed_dim, proj_dim)
        self.text_proj = nn.Linear(text_embed_dim, proj_dim)

        self.bias = self.create_parameter(
            shape=[1], attr=ParamAttr(regularizer=L2Decay(0.0)))
    ...
    def _reset_parameters(self):
        linear_init_(self.visual_proj)
        linear_init_(self.text_proj)
        bias_cls = bias_init_with_prob(0.01)
        constant_(self.bias, bias_cls)
        constant_(self.scale, math.log(1 / 0.07))

    def forward(self, visual_feat, text_feat, text_token_mask=None):
        visual_feat = self.visual_proj(visual_feat)
        text_feat = self.text_proj(text_feat)
        visual_feat = F.normalize(visual_feat, p=2, axis=-1)
        text_feat = F.normalize(text_feat, p=2, axis=-1)
        logit = paddle.matmul(visual_feat, text_feat, transpose_y=True)
        logit = logit * self.scale.exp() + self.bias
        if text_token_mask is not None:
            logit.masked_fill_(~text_token_mask[:, None, :], float('-inf'))
        return logit
...
class OVRTDETRTransformer(nn.Layer):
    ...
    def __init__(self,
        ...
        self.enc_score_head = ContrastiveEmbed(hidden_dim,
                                               text_embed_dim,
                                               proj_dim=512)
        ...
        self.dec_score_head = nn.LayerList([
            ContrastiveEmbed(hidden_dim, text_embed_dim, proj_dim=512)
    ...
    def forward(self,
        ...
        target, init_ref_points_unact, enc_topk_bboxes, enc_topk_logits = \
            ...
            memory, text_feats, text_token_mask, spatial_shapes, denoising_class, denoising_bbox_unact,is_teacher)
    ...
    def _get_decoder_input(self,
        ...
        enc_outputs_class = self.enc_score_head(output_memory, text_tgt,
                                                text_token_mask)
