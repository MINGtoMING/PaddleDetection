...
from .utils import (get_valid_ratio, get_contrastive_denoising_training_group,
                    inverse_sigmoid)
from ..heads.detr_head import MLP
from ..initializer import (linear_init_, constant_, xavier_uniform_, normal_,
                           bias_init_with_prob)
from .dino_transformer import DINOTransformerDecoderLayer, DINOTransformerDecoder
from ..shape_spec import ShapeSpec

__all__ = ['CoDINOTransformer']


@register
class CoDINOTransformer(nn.Layer):
    ...
    def __init__(self,
        ...
        assert len(in_feats_channel) == len(in_feats_channel)
        assert len(in_feats_channel) <= num_levels

        self.hidden_dim = hidden_dim
        self.nhead = nhead
        self.in_feats_stride = in_feats_stride
        ...
        self.downsample = nn.Sequential(
            nn.Conv2D(hidden_dim,
                      ...
                      kernel_size=3,
                      stride=2,
                      padding=1), nn.GroupNorm(32, hidden_dim))
    ...
    def from_config(cls, cfg, input_shape):
        return {
            ...
            'in_feats_stride': [i.stride for i in input_shape],
        }
    ...
    def _get_encoder_input(self, feats, pad_mask=None):
        ...
        # [b, l]
        mask_flatten = None if pad_mask is None else paddle.concat(
            mask_flatten, 1)
        # [b, l, c]
    ...
    def forward(self, feats, pad_mask=None, gt_meta=None):
        ...
        # encoder outs for aux heads
        aux_outs = memory.split([h * w for h, w in spatial_shapes],
                                axis=1)
        aux_outs = [
            out.transpose([0, 2, 1]).reshape([0, 0, h, w])
            for out, (h, w) in zip(aux_outs, spatial_shapes)
        ]
        aux_outs.append(self.downsample(aux_outs[-1]))
    ...
    def _get_encoder_output_anchors(self,
        ...
        for lvl, (h, w) in enumerate(spatial_shapes):
            ...
            grid_y, grid_x = paddle.meshgrid(paddle.arange(end=h),
                                             paddle.arange(end=w))
            grid_xy = paddle.stack([grid_x, grid_y], -1).astype(memory.dtype)

            valid_WH = paddle.stack([valid_W, valid_H],
                                    -1).reshape([-1, 1, 1,
                                                 2]).astype(grid_xy.dtype)
    ...
    @property
    def aux_out_shape(self):
        strides = self.in_feats_stride
        for _ in range(len(self.in_feats_stride), self.num_levels + 1):
            strides.append(strides[-1] * 2)
        return [
            ShapeSpec(
                channels=self.hidden_dim, stride=strides[idx])
